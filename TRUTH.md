...
- notification card now uses the packaged SLOPOS mark, but its compact composition still needs independent classic-Mac review;
...
- In a fresh Rust 1.97 Linux container, `cargo metadata --locked`, `cargo fmt --all -- --check`, `cargo check --workspace --locked`, `cargo test --workspace --locked` and `cargo clippy --workspace --all-targets --locked -- -D warnings` all exited 0. The test run reports 13 catalogue, 5 shell unit, 24 shell integration and 3 spaces tests, with 0 failures after adding the packaged Settings icon contract.
...
- The no-fork browser integration was checked in the disposable Arch application gate: Chromium launched through the wrapper with the optional unpacked SLOPOS theme and a fresh 1280x800 visible frame; Firefox launched through the same X11 wrapper with a disposable explicit profile, backed-up `userChrome.css`/`user.js` changes and a fresh PID-matched 1280x800 frame. The wrapper now clears `WAYLAND_DISPLAY`, forces GTK/Firefox to X11 and adds Chromium-family `--ozone-platform=x11`; it never rewrites a browser binary or web content. The Firefox frame includes the expected container-only sandbox warning because user namespaces are denied; it is not treated as product UI acceptance. The Figma Classic Macintosh UI Kit supplied by the owner was captured and used as the visual comparison reference; upstream browser chrome remains only best-effort aligned.
- The shared GTK theme now styles native `menubar > menuitem` spacing, hover and focus states. Fresh PCManFM and Xfce Terminal captures show separated Platinum menu labels; this same GTK surface is available to browser-native dialogs/menus through the no-fork wrapper, while browser tab, address-bar and web-content styling remain engine/profile-owned.
