...
| Upstream application integration | 10 | **9/10** | The retained pre-hardening Arch/X11 matrix launched five distinct upstream roles—PCManFM, Xfce Terminal, Mousepad, Ristretto and Chromium—and captured 1280x800 scenes; its SuperTux evidence stopped at the title menu plus a null-sink stream. The current gate now requires a deterministic local browser fixture and a packaged SuperTux world1 level with movement/jump input, but this revised gate must be rerun before its evidence replaces the older artifacts. Chromium inherits the SLOPOS GTK/XDG identity and optional Platinum frame theme without a fork; Firefox has an explicit, backed-up profile stylesheet path. Hardware audio and full upstream-window visual convergence remain unreviewed. |
...
6. **Browser/game boundary:** The source gate now exercises a local browser page and a real SuperTux level/input path, but the revised Arch run is still pending. A browser can only inherit the SLOPOS GTK/frame language through upstream extension/profile mechanisms. Physical game audio, GPU behavior and complete browser-content styling still require hardware/independent review.
...
- The retained Arch upstream application gate evidence predates the current browser/game hardening: it launched PCManFM, Xfce Terminal, Mousepad, Ristretto and Chromium through `start-slopos-browser`, then launched SuperTux with a 960x540 window and the real title menu. Each launched window matched its PID; six fresh 1280x800 PNGs were non-empty, and `pactl list sink-inputs` contained an identifiable SuperTux stream on the disposable PulseAudio null sink. The revised gate now requires a local browser DOM/visible-title assertion, direct world1 level input, orderly game shutdown and fatal-log rejection; rerun evidence is pending. This remains bounded container evidence, not physical speaker or GPU evidence.
