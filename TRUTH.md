...
- The no-fork browser integration was checked in the disposable Arch application gate: Chromium launched through the wrapper with the optional unpacked SLOPOS theme and a fresh 1280x800 visible frame; Firefox receives backed-up `userChrome.css`/`user.js` changes only when an explicit absolute profile path is supplied. The wrapper now clears `WAYLAND_DISPLAY`, forces GTK/Firefox to X11 and adds Chromium-family `--ozone-platform=x11`; it never rewrites a browser binary or web content. The Figma Classic Macintosh UI Kit supplied by the owner was captured and used as the visual comparison reference; upstream browser chrome remains only best-effort aligned.
