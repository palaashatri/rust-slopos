...
apt-get install -y -qq --no-install-recommends \
  ...
  ca-certificates adwaita-icon-theme fonts-liberation fonts-dejavu-core
...
dbus-run-session -- ./target/release/slopos-session >artifacts/qa/session.log 2>&1 &
SESSION_PID=$!
...
xdotool search --onlyvisible --name '^SLOPOS Search$' >/dev/null
